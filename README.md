...
- **Prisma ORM** — робота з базою даних.
    1. Node.js - багатофункціональне середовище виконання JavaScript. Використовується для створення backend-логіки та асинхронної роботи (наприклад, запити до бази даних).
    Без цього компонента неможливе запуск серверної частини програми
...
<summary>Натисніть, щоб побачити пояснення до структури цього проекту<summary>
    ...
    <summary>Натисніть, щоб побачити пояснення кожного файла модулю у папці src</summary>
        router - відповідає за отримання запитів та їх передачу до контролера
        controller - відповідає за обробку та валідацію запиту, отримує дані із сервера та складає відповідь
        service - відповідає за реалізацію бізнес-логіки програми, тобто її головні завдання
        repository - надає доступ до роботи з базою даних
        types - відповідає за строгу типізацію даних, струкуризує отримані функціями дані та їх відповідь
